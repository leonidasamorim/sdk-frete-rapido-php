...
beforeEach(function () {
    ...
    $this->ShippingCostV3 = new MockHandler([
        new Response(200, [], $this->jsonMock("ShippingCostV3")),
    ]);
...
test("should search for offers to shipping cost in json format", function () {
    ...
    $this->freteRapido = new Client($this->auth, ['handler' => $this->ShippingCostV3]);

    $result = $this->freteRapido->shippingCostV3()->calculate($this->args)->get();
...
test("should search for offers to shipping cost in array format", function () {
    ...
    $result = $this->freteRapido->shippingCostV3()->calculate($this->args)->getArray();
